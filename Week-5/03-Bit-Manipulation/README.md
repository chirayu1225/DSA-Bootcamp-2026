...
[Home](../../README.md) > [Week 5](../README.md) > Bit Manipulation

> Week 5 · Topic 3 of 4 · Prerequisites: [Time & Space Complexity](../../Week-1/02-Complexity/README.md)

---

## Why This Topic Now

Every piece of data is ultimately stored as binary sequences (`0`s and `1`s). By operating directly via the CPU's Arithmetic Logic Unit (ALU), bitwise algorithms execute in a single clock cycle, bypassing high-level arithmetic overhead. Once you see the patterns - XOR, masking, shifts - a whole class of problems that previously required hash maps or extra memory becomes trivial to solve in O(1) space.
...
Most modern operating systems and processing units rely heavily on Two's Complement to represent negative integers because of its elegant structural math.
...
- _43 = 00101011 in binary_
- Invert bits (_∼43_): _11010100_
- Add 1: _11010101_ (which equals _-4310_)

Why do systems use this? It unifies standard subtraction and addition operations into the exact same hardware circuit loop. The ALU can ignore signed vs. unsigned identities during integer addition, meaning it does not need a separate logic circuit for subtraction.

## Overflow Dynamics

When an operation exceeds bounded limits:
...
- _Crucial mathematical properties for tracking states:_
  ...
  - Commutative: _A ^ B = B ^ A_
  - Associative: _A ^ (B ^ C) = (A ^ B) ^ C_
  - Self-Inverse identity: _X ^ X = 0_
  - Identity element: _X ^ 0 = X_
...
- How it works: Subtracting `1` from a binary number flips all the bits from the right up to the first `1` it encounters. Combining `n` with `n-1` via a bitwise AND cleanly eliminates that rightmost `1`.
...
1. Every element appears twice except for one: XORing all the numbers together cancels out the duplicates (_X ^ X = 0_), leaving behind the single unique value.

2. Every element appears twice except for two unique numbers (_x_ and _y_): XORing all elements yields `xor_xy = x ^ y`. Since _x_ and _y_ are distinct, `xor_xy` must have at least one set bit. You can isolate this bit using `xor_xy & -xor_xy`, and then use it as a mask to split the array into two groups. XORing each group separately isolates _x_ and _y_.
...
| Operational Feature | C++ Built-in (GCC) | Java Standard Library | Python Bit Methods |
|---|---|---|---|
| Count Active Set Bits | `__builtin_popcount(x)` | `Integer.bitCount(x)` | `x.bit_count()` |
| Count Leading Zeros | `__builtin_clz(x)` | `Integer.numberOfLeadingZeros(x)` | Manual Calculation |
| Count Trailing Zeros | `__builtin_ctz(x)` | `Integer.numberOfTrailingZeros(x)` | Manual Calculation |
...
- Can you implement all 4 bitmask operations (get, set, clear, toggle) from memory?
- Can you use XOR to find the single non-duplicate element in an array without a hash map?
- Do you understand why `n & (n-1)` clears the lowest set bit?
- Can you generate all subsequences of an array using bitmasks?

---

## Resources

- [All About Bit Manipulation - GeeksforGeeks](https://www.geeksforgeeks.org/dsa/all-about-bit-manipulation/)
- [Bit Manipulation Important Tactics - GeeksforGeeks](https://www.geeksforgeeks.org/dsa/bits-manipulation-important-tactics/)
- [Algebra: Bit Manipulation Techniques - CP-Algorithms](https://cp-algorithms.com/algebra/bit-manipulation.html)
- [All Types of Patterns for Bit Manipulation - LeetCode Discuss](https://leetcode.com/discuss/post/3695233/all-types-of-patterns-for-bits-manipulat-qezp/)

---

[Week 5 Overview](../README.md) | [Previous: Greedy Algorithms](../02-Greedy/README.md) | [Next: Number Theory](../04-Number-Theory/README.md)
